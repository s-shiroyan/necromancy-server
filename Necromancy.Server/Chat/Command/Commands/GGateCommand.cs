...
namespace Necromancy.Server.Chat.Command.Commands
{
    ...
    public class GGateCommand : ServerChatCommand
    {
            ...
            List<ChatResponse> responses)
        {
            ...
            switch (command[0])
            {
                case "spawn": //spawns a new object on the map at your position.  makes a sign above it.  and jumps over it
                    ...
                    res.WriteInt32(myGGateSpawn.InstanceId); // Unique Object ID.  Crash if already in use (dont use your character ID)
                    ...
                    res.WriteCString($"The Instance ID of your GGateSpawn is: {myGGateSpawn.InstanceId}"); //"0x5B" //Title
                    ...
                    res.WriteInt32(myGGateSpawn.ModelId); // Optional Model ID. Warp Statues. Gaurds, Pedistals, Etc., to see models refer to the model_common.csv
                    res.WriteInt16(myGGateSpawn.Size); //  size of the object
                    res.WriteInt32(myGGateSpawn.Active); // 0 = collision, 1 = no collision  (active/Inactive?) //rename this to state...... >.>
                    res.WriteInt32(myGGateSpawn.Glow); //0= no effect color appear, //Red = 0bxx1x   | Gold = obxxx1   |blue = 0bx1xx
                    ...
                    if (command[2] == "add")  //if you want to send your GGateSpawn straight to the DB.  type Add at the end of the spawn command. 
                    {
                        ...
                        { responses.Add(ChatResponse.CommandError(client, $"Added GGateSpawn {myGGateSpawn.Id} to the database")); }
                ...
                case "move": //move a GGateSpawn to your current position and heading
                    ...
                    myGGateSpawn.Z = client.Character.Z;
                    myGGateSpawn.Heading = client.Character.Heading;
                    res.WriteInt32(myGGateSpawn.InstanceId);
                    res.WriteFloat(client.Character.X);
                    res.WriteFloat(client.Character.Y);
                ...
                case "update": //Updates an existing entry in the database
                    ...
                    { responses.Add(ChatResponse.CommandError(client, $"Updated GGateSpawn {myGGateSpawn.Id} in the database")); }
                    break;
                case "remove": //removes a GGateSpawn from the database
                    myGGateSpawn = Server.Instances.GetInstance((uint)x) as GGateSpawn;
                    if (!Server.Database.DeleteGGateSpawn(myGGateSpawn.Id))
                    {
                        responses.Add(ChatResponse.CommandError(client, "myGGateSpawn could not be deleted from database"));
                        return;
                    }
                    else
                    { responses.Add(ChatResponse.CommandError(client, $"Removed GGateSpawn {myGGateSpawn.Id} from the database")); }
                    res.WriteInt32(myGGateSpawn.InstanceId);
                ...
                case "state": //updates GGate State.
	                res.WriteInt32(myGGateSpawn.InstanceId);
                    res.WriteInt32(y);
                    Router.Send(client.Map, (ushort)AreaPacketId.recv_npc_ggate_state_update_notify, res, ServerType.Area);
                    break;
                default: //you don't know what you're doing do you?
                    Logger.Error($"There is no recv of type : {command[0]} ");
                    { responses.Add(ChatResponse.CommandError(client, $"{command[0]} is not a valid GGateSpawn command.")); }
                    break;
        ...
        public override string HelpText => "usage: `/GGateSpawn [argument] [number] [parameter]` - does something GGateSpawn related";
