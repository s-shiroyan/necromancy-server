...
namespace Necromancy.Server.Chat.Command.Commands
{
    ...
    public class PlayersCommand : ServerChatCommand
    {
            ...
            List<ChatResponse> responses)
        {
            if (command[0] == null)
            {
                responses.Add(ChatResponse.CommandError(client, $"Hi There!  Type /players world or /players map to see who's here.  You can also type a Character.Name"));
                return;
            }
            ...
            switch (command[0])
            {
                case "map": //tells you all the people on the Map you're on
                    ...
                    {
                        responses.Add(ChatResponse.CommandError(client, $"{theirClient.Character.Name} {theirClient.Character.SoulName} is on Map {theirClient.Character.MapId} with InstanceID {theirClient.Character.InstanceId}"));
                    }
                    break;
                ...
                default: //you don't know what you're doing do you?
                    ...
                    foreach (Character theirCharacter in Server.Characters.GetAll())
                    {
                        ...
                        {
                            responses.Add(ChatResponse.CommandError(client, $"{theirCharacter.Name} {theirCharacter.SoulName} is on Map {theirCharacter.MapId} with InstanceID {theirCharacter.InstanceId}"));
                            soulFound = true;
                        }
                    }
                    if (soulFound == false)
                    {
                        ...
                        responses.Add(ChatResponse.CommandError(client, $"{command[0]} is not a valid players command.")); 
                    }
                    break;                    
            }
        }
