...
namespace Necromancy.Server.Packet.Area
{
    public class send_map_get_info : ClientHandler
    {
        ...
        private void SendDataNotifyNpcData(NecClient client)
        {
            ...
            string[][]
                jaggedArray =
                    FileReader.GameFileReader(
                        client); //Reads a manually created CSV file to load NPCs per Client.Character.MapID based on settings from map_symbol.csv and npc.csv
            foreach (string[] ary1 in jaggedArray)
            {
                npcCount++;
                Logger.Debug(
                    $"Loading NPC Number: {npcCount} from TempNPCData.csv with values {ary1[1]}, {ary1[2]}, {ary1[3]}, {ary1[4]}, {ary1[5]}, {ary1[6]}, {ary1[7]},");
                ...
                res2.WriteByte(
                    0); // 0 - Clickable NPC (Active NPC, player can select and start dialog), 1 - Not active NPC (Player can't start dialog)
                ...
                res2.WriteInt32(
                    numEntries2); // number of status effects.  128 Max.   setting to 0 for noise reduction in console
