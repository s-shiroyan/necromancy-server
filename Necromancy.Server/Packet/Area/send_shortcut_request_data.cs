...
namespace Necromancy.Server.Packet.Area
{
    public class send_shortcut_request_data : ClientHandler
    {
        ...
        public override void Handle(NecClient client, NecPacket packet)
        {
            ...
            Logger.Debug(client, $"client.Character.shortcutBar0Id {client.Character.shortcutBar0Id}  client.Character.shortcutBar1Id {client.Character.shortcutBar1Id}");
            ...
            for (int i = 0; i < shortcutBar0.Length; i++)
            {
                if (shortcutBar0[i] > 0)
                {
                    ...
                    Logger.Debug(client, $"shortcut bar slot {i} skillId {shortcutBar0[i]}");
                    res0.WriteByte(0);
                    res0.WriteByte((byte)i);
                    if (shortcutBar0[i] > 100)   // Action type?    3 for skill_tree item, 5 for emote
                    {
                        res0.WriteInt32(3);
                    } else
                    {
            ...
            for (int i = 0; i < shortcutBar1.Length; i++)
            {
                if (shortcutBar1[i] > 0)
                {
                    ...
                    if (shortcutBar1[i] > 100)  // Action type?    3 for skill_tree item, 5 for emote
                    {
                        res1.WriteInt32(3);
                    }
                    else
