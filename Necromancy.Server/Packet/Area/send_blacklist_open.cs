...
namespace Necromancy.Server.Packet.Area
{
    public class send_blacklist_open : ClientHandler
    {
        ...
        public override void Handle(NecClient client, NecPacket packet)
        {
            ...
            if (TempCharacterCount > 10 ) { TempCharacterCount = 10; }
            ...
            //for (int i = 0; i < 10; i++)
            foreach (Character blackCharacter in Server.Characters.GetAll()) //Max 10 Loops. will break if more than 10 characters in Db.
            {
                Soul blackSoul = Server.Database.SelectSoulById(blackCharacter.SoulId);

                res.WriteInt32(blackCharacter.InstanceId); //Unique Row ID for blacklist.  using Character InstanceID, but can be anything
                ...
                res.WriteInt32(blackCharacter.InstanceId); //for online location tracking?
                res.WriteFixedString(blackCharacter.Name, 91); //character name of BlackListMember
                res.WriteInt32(blackCharacter.ClassId); // Character Class of BlackListMember
