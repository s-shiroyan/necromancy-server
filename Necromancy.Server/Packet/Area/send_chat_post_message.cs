...
namespace Necromancy.Server.Packet.Area
{
    public class send_chat_post_message : ClientHandler
    {
        ...
        private void ChangeFormMenu(NecClient client)
        {
            ...
            int SexInteger = int.Parse(SexString); */
            bool isFemale = false;
            bool isMale = false;
            ...
            Router.Send(client, (ushort)AreaPacketId.recv_event_start, res2, ServerType.Area);
                ...
                IBuffer res70 = BufferProvider.Provide();
                    ...
                    Router.Send(client, (ushort)AreaPacketId.recv_event_select_push, res70, ServerType.Area);   // It's the first choice
            ...
            res1.WriteInt32(0); // Don't know what it affect
            ...
            res.WriteInt32(2); // gender 1 = female, 2 = male
            ...
            res6.WriteInt32(client.Character.Raceid); // race
            res6.WriteInt32(1); // gender 1 = female, 2 = male
            res6.WriteByte(client.Character.HairId); ; //hair
            res6.WriteByte(client.Character.HairColorId); //color
            res6.WriteByte(client.Character.FaceId); //face
            Router.Send(client, (ushort)AreaPacketId.recv_chara_update_form, res6, ServerType.Area);
            ...
            /*IBuffer res = BufferProvider.Provide();
                   ...
                   /* IBuffer res1 = BufferProvider.Provide();
                    ...
                    res1.WriteByte(0);
                    res1.WriteByte(0);
                    Router.Send(client, (ushort)AreaPacketId.recv_chara_update_form, res1, ServerType.Area); */
...
/*
        ...
        private void AdminConsoleRecvItemInstanceUnidentified(NecClient client)
        {
            ...
            for (i = 0; i < 19; i++)
            {
                ...
                IBuffer res12 = BufferProvider.Provide(); // Number of item (same) you got (example 5 potion)
                res12.WriteInt64(itemIDs[x]);
                res12.WriteByte((byte)Util.GetRandomNumber(0, 100)); // Random number attribute
                Router.Send(client, (ushort)AreaPacketId.recv_item_update_num, res12, ServerType.Area);

                IBuffer res16 = BufferProvider.Provide();
                res16.WriteInt64(itemIDs[x]);
                res16.WriteByte((byte)Util.GetRandomNumber(0, 100)); // Random item level
                Router.Send(client, (ushort)AreaPacketId.recv_item_update_level, res16, ServerType.Area);


                IBuffer res18 = BufferProvider.Provide();
                res18.WriteInt64(itemIDs[x]);
                res18.WriteByte((byte)Util.GetRandomNumber(0, 100)); // Random level for item Special forge
                Router.Send(client, (ushort)AreaPacketId.recv_item_update_sp_level, res18, ServerType.Area);


                IBuffer res1 = BufferProvider.Provide();
                res1.WriteInt64(itemIDs[x]); //itemIDs[x]   put stuff unidentified and get the status equipped  , 0 put stuff identified
                res1.WriteInt32(0); // 1 = item "eqquiped" status but unidentified  follow the pattern 2 4 6 8, 8 = cursed
                Router.Send(client, (ushort)AreaPacketId.recv_item_update_state, res1, ServerType.Area);


                IBuffer res28 = BufferProvider.Provide();
                res28.WriteInt64(itemIDs[x]); // item id

                res28.WriteByte(0); // 0 = adventure bag. 1 = character equipment, 2 = royal bag
                res28.WriteByte(0); // position 2	cause crash if you change the 0	]	} im assumming these are x/y row, and page
                res28.WriteInt16((short)x); // bag index 0 to 24
                Router.Send(client, (ushort)AreaPacketId.recv_item_update_place, res28, ServerType.Area);
        ...
        private void AdminConsoleRecvItemInstance(NecClient client)
        {
            ...
            for (int j = 0; j < 19; j++)
            {
                ...
                //recv_item_instance = 0x86EA,
                x++;
                res.WriteInt64(itemIDs[x]); //  Assume Unique ID instance identifier. 1 here makes item green icon
        ...
        int[] itemIDs = new int[]
        {
            10300206 /*Weapon*/, 15100901 /*Shield* */, 20000101 /*Arrow*/, 110301 /*head*/, 210701 /*Torso*/,
        ...
        int[] EquipItemType = new int[]
            {5, 20, 23, 28, 31, 32, 36, 40, 41, 44, 43, 45, 42, 54, 61, 61, 61, 61, 61, 61, 0, 0};
