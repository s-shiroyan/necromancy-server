...
namespace Necromancy.Server.Packet.Msg
{
    public class send_chara_get_list : Handler
    {
        ...
        private void SendNotifyData(NecClient client)
        {
            ...
            if (Database.SelectCharacterBySoulId(client.Character.SoulId) != null)
            {
                ...
                foreach (Character myCharacter in Database.SelectCharacterBySoulId(client.Character.SoulId))
                {
                    ...
                    res.WriteInt32(myCharacter.ClassId);//class stat 
                                      //
                    res.WriteInt32(myCharacter.Raceid);//race flag
                    res.WriteInt32(myCharacter.Sexid);//gender flag
                    res.WriteByte(myCharacter.HairId);//Hair Style
                    res.WriteByte(myCharacter.HairColorId);//Hair Color
                    res.WriteByte(myCharacter.FaceId);//Face Style
            ...
            ///Failover for testing.  If blank database, populate some defaults 
            if (Database.SelectCharacterBySoulId(client.Character.SoulId) == null)
            {
                int E = 0;
