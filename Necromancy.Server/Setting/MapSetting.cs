...
namespace Necromancy.Server.Setting
{
    public class MapSetting
    {
        ...
        public static string[] MapLoadInfo(int MapId)
        {
            string[] mapLoadInfo = new string[18];

            for (int i = 0; i < DumpLines.Length; i++)
            {
                string matchCheck = DumpLines[i];
                mapLoadInfo = matchCheck.Split(',');
                //Console.WriteLine($"MapLoadInfo : {mapLoadInfo[0]}");
                
                if ($"{mapLoadInfo[0]}" == $"{MapId}")
                {
                    Console.WriteLine($"Matched : {mapLoadInfo[5]} for mapID {MapId}");
                    return mapLoadInfo;
                    
                }
            }
            Console.WriteLine($"Could not Match mapID {MapId}  Setting Default Entry point of : Deep Sea Port");
            mapLoadInfo = new string[] { "1001002", "1001002", "1001002", "Kingdom of Dimento", "Illfalo Port", "Deep Sea Port", "23162", " -219", " -7", " 260", " 2", " 0", "1", "1", "2", "1001002", "0" };         
            return mapLoadInfo;
        }

        static string Dump = @"
#,ReferID,,Map name,,,Coordinates at map change,,,,BGM-ID,,Town,Overall map,,Loading,
#MapID,,,,,,,,,,,,,,,Image ID,Number of map divisions
...
9009011,9009011,9009011, Cave Map Mock,,,0,0,30,270,11,0,0,,,2001151,1
            ";
        static string[] DumpLines = Dump.Split('\n');
